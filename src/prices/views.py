...
def chart_one_belarus():            # для расчета данных к таблице - Кол-во собранных данных о конкурентахс маркет-плейсов 
    def execute_look():
        ...
        #print('FFFINAL COUNTDOWN', final_parsed_data_from_sites_whole, final_parsed_data_from_sites, date_to_look_parsed_data ) #210 [['Сайт', 'Количество спарсенных конкурентов'], ['onliner.by', 20], ['bagoria.by', 147], ['autoset.by', 43]] 06.01.2025
    ...
    final_parsed_data_from_sites_whole = []
    date_to_look_parsed_data = []
    final_parsed_data_from_sites = []
    what_to_add_to_json = {}
    date_to_look_parsed_data = datetime.datetime.now().date()
    date_to_look_parsed_data_indate_type = date_to_look_parsed_data
    if date_to_look_parsed_data is datetime.date:
        date_to_look_parsed_data.strftime('%d.%m.%Y')
    if models.COMPETITORS_DATE_FROM_USER_ON_FILTER:
        date_to_look_parsed_data = models.COMPETITORS_DATE_FROM_USER_ON_FILTER[0]
        date_to_look_parsed_data = datetime.datetime.strptime(date_to_look_parsed_data, '%Y-%m-%d').date()
        ...
        date_to_look_parsed_data = date_to_look_parsed_data.strftime('%d.%m.%Y')
    try:
            #блок для формирования словаря во время парсинга ------- ПАРСИНГ
        if not models.AllCompetitorSiteModelDict.objects.exclude(chart_one_json={}).filter(chart_one_json__isnull=False):    # если на автомате роходит парсинг и на дату нет - сформировать  
            final_parsed_data_from_sites_whole, final_parsed_data_from_sites, date_to_look_parsed_data = execute_look()
            what_to_add_to_json = {}
            what_to_add_to_json[date_to_look_parsed_data] = final_parsed_data_from_sites_whole, final_parsed_data_from_sites
            if not what_to_add_to_json:
                pass
            else:
                chart_one_json_obj, iss_created = models.AllCompetitorSiteModelDict.objects.get_or_create(chart_one_json=what_to_add_to_json) 
                chart_one_json_obj.chart_one_json = what_to_add_to_json
                chart_one_json_obj.save()
                #print('CHOTOSDELAL', models.AllCompetitorSiteModelDict.objects.values_list('chart_one_json', flat=True))
    except: 
        pass
             # ЕСЛИ ДАННЫЕ ЕСТЬ   ----- НЕ ПАРСИНГ  # ВЕРНУТЬ EXCEPT
    # блок по созданию отпечатка всех спарсенных данных на дату:
    what_to_add_to_json = {}
    try:        # ПОЛУЧИТЬ ДАННЫЕ НА ВЫБРАННУЮ ДАТУ           
        data_to_return = models.AllCompetitorSiteModelDict.objects.exclude(chart_one_json={}).filter(chart_one_json__isnull=False).values_list('chart_one_json', flat=True)#.filter(chart_one_json__contains=[date_to_look_parsed_data]) # СПИСОК С ДАННЫМИ НА ДАТУ ИЗ СЛОВАРЯ  
        #print('DATA--', date_to_look_parsed_data)
        #print('ЖЖЖ',  data_to_return)
        data_to_return = list(data_to_return)#[0]
            ...
            try:
                if n[date_to_look_parsed_data]:
                    ...
                    if data_to_return:
                        ...
                        final_parsed_data_from_sites_whole = data_to_return[0]
                        final_parsed_data_from_sites = data_to_return[1]
                        #print('П--АПА', final_parsed_data_from_sites_whole, date_to_look_parsed_data, final_parsed_data_from_sites)
                        break
            except: 
                pass
    except: 
        pass
    if not final_parsed_data_from_sites_whole or not final_parsed_data_from_sites: # если на текущую дату нет данных - 1) ПОПЫТАТЬСЯ СФОРМИРОВАТЬ СЛОВАРЬ: 
        final_parsed_data_from_sites_whole, final_parsed_data_from_sites, date_to_look_parsed_data = execute_look() 
        #print('USRATSJA2 func1')
        what_to_add_to_json = {}
        what_to_add_to_json[date_to_look_parsed_data] = final_parsed_data_from_sites_whole, final_parsed_data_from_sites
        if not what_to_add_to_json or not final_parsed_data_from_sites_whole or not final_parsed_data_from_sites:
            #try: # если на конкретную дату инфы нет - взять ближайшую следующую БЛИЖАЙШУЮ ДАТУ В СПИСКЕ в списке:
                data_to_return = models.AllCompetitorSiteModelDict.objects.exclude(chart_one_json={}).filter(chart_one_json__isnull=False).values_list('chart_one_json', flat=True)#.filter(chart_one_json__contains=[date_to_look_parsed_data]) # СПИСОК С ДАННЫМИ НА ДАТУ ИЗ СЛОВАРЯ  
                data_to_return = list(data_to_return)#[0]
                list_dates_keys = []    # список со всеми доступными датами
                for n in list(data_to_return):
                    if datetime.datetime.strptime(list(dict(n).keys())[0], '%d.%m.%Y').date() != date_to_look_parsed_data_indate_type:
                        #print('=++++++', datetime.datetime.strptime(list(dict(n).keys())[0], '%d.%m.%Y'), '------', date_to_look_parsed_data)
                        list_dates_keys.append(datetime.datetime.strptime(list(dict(n).keys())[0], '%d.%m.%Y').date())
                #print('list_dates_keys', list_dates_keys)
                closest_date = min(list_dates_keys, key=lambda x: (x>date_to_look_parsed_data_indate_type, abs(x-date_to_look_parsed_data_indate_type)))
                date_to_look_parsed_data = closest_date.strftime('%d.%m.%Y')    # БЛИЖАЙШАЯ НАЙДЕННАЯ ДАТА 
                data_to_return = models.AllCompetitorSiteModelDict.objects.exclude(chart_one_json={}).filter(chart_one_json__isnull=False).values_list('chart_one_json', flat=True)#.filter(chart_one_json__contains=[date_to_look_parsed_data]) # СПИСОК С ДАННЫМИ НА ДАТУ ИЗ СЛОВАРЯ  
                data_to_return = list(data_to_return)#[0]
                for n in list(data_to_return):                  #ПОИСК НА БЛИЖАЙШУЮ НАЙДЕННУЮ ДАТУ
                    try:
                        data_to_return_is = None
                        if n[date_to_look_parsed_data]:
                            data_to_return_is = n[date_to_look_parsed_data]
                            if data_to_return_is:
                                date_to_look_parsed_data = date_to_look_parsed_data
                                final_parsed_data_from_sites_whole = data_to_return_is[0]
                                final_parsed_data_from_sites = data_to_return_is[1]
                                break        
                    except: 
                        pass
                if final_parsed_data_from_sites_whole:
                    #print('KO-KO-KO', final_parsed_data_from_sites_whole, date_to_look_parsed_data, final_parsed_data_from_sites)
                    models.CLOSEST_DATA_FOUND1 = date_to_look_parsed_data_indate_type
                    #print('models.CLOSEST_DATA_FOUND1 = ', models.CLOSEST_DATA_FOUND1) 
                    return final_parsed_data_from_sites_whole, final_parsed_data_from_sites, date_to_look_parsed_data
            #except:
            #    pass
        else:
    ...
    #print('I CHO TAM', final_parsed_data_from_sites_whole, date_to_look_parsed_data, final_parsed_data_from_sites) 
    models.CLOSEST_DATA_FOUND1 = None 
    return final_parsed_data_from_sites_whole, final_parsed_data_from_sites, date_to_look_parsed_data  # 210 [['Сайт', 'Количество спарсенных конкурентов'], ['onliner.by', 20], ['bagoria.by', 147], ['autoset.by', 43]] 06.01.2025
    #(210, [['Сайт', 'Количество спарсенных конкурентов'], ['onliner.by', 20], ['bagoria.by', 147], ['autoset.by', 43]], '06.01.2025')
...
def chart_two_belarus():
    def execute_look():
        ...
        #print('==1!!!!!!!!!!!', top_brands_counter_for_chart)
        date_to_look_parsed_data = date_to_look_parsed_data.strftime('%d.%m.%Y')
        #date_to_look_parsed_datas1 = list_of_parrsed_brands_sites
        models.DATE_TO_LOOK_PARSED_DATAS = list_of_parrsed_brands_sites 
    ...
    try:
            #блок для формирования словаря во время парсинга ------- ПАРСИНГ
        if not models.AllCompetitorSiteModelDict.objects.exclude(chart_two_json={}).filter(chart_two_json__isnull=False):    # если на автомате роходит парсинг и на дату нет - сформировать  
            top_brands_counter_for_chart, date_to_look_parsed_data, list_of_parrsed_brands_sitess = execute_look()
            what_to_add_to_json = {}
            what_to_add_to_json[date_to_look_parsed_data] = top_brands_counter_for_chart, list_of_parrsed_brands_sites
            if not what_to_add_to_json:
                pass
            else:
                chart_two_json_obj, iss_created = models.AllCompetitorSiteModelDict.objects.get_or_create(chart_two_json=what_to_add_to_json) 
                chart_two_json_obj.chart_two_json = what_to_add_to_json
                chart_two_json_obj.save()
                #print('CHOTOSDELAL', models.AllCompetitorSiteModelDict.objects.values_list('chart_two_json', flat=True))
    except: 
        pass
             # ЕСЛИ ДАННЫЕ ЕСТЬ   ----- НЕ ПАРСИНГ  # ВЕРНУТЬ EXCEPT
    # блок по созданию отпечатка всех спарсенных данных на дату:
    what_to_add_to_json = {}
    try:        # ПОЛУЧИТЬ ДАННЫЕ НА ВЫБРАННУЮ ДАТУ           
        data_to_return = models.AllCompetitorSiteModelDict.objects.exclude(chart_two_json={}).filter(chart_two_json__isnull=False).values_list('chart_two_json', flat=True)#.filter(chart_two_json__contains=[date_to_look_parsed_data]) # СПИСОК С ДАННЫМИ НА ДАТУ ИЗ СЛОВАРЯ  
        #print('DATA--', date_to_look_parsed_data)
        #print('ЖЖЖ',  data_to_return)
        data_to_return = list(data_to_return)#[0]
        for n in list(data_to_return):
            ...
            try:
                if n[date_to_look_parsed_data] and n[date_to_look_parsed_data][1]:
                    ...
                    if data_to_return:
                        ...
                        #print('П--АПА', top_brands_counter_for_chart, date_to_look_parsed_data, list_of_parrsed_brands_sites)
                        break
            except: 
                pass
    except: 
        pass
    if not top_brands_counter_for_chart or not list_of_parrsed_brands_sites: # если на текущую дату нет данных - 1) ПОПЫТАТЬСЯ СФОРМИРОВАТЬ СЛОВАРЬ: 
        top_brands_counter_for_chart, date_to_look_parsed_data, list_of_parrsed_brands_sites = execute_look() 
        #print('USRATSJA2')
        what_to_add_to_json = {}
        what_to_add_to_json[date_to_look_parsed_data] = top_brands_counter_for_chart, list_of_parrsed_brands_sites
        if not what_to_add_to_json or not top_brands_counter_for_chart or not list_of_parrsed_brands_sites:
            try: # если на конкретную дату инфы нет - взять ближайшую следующую БЛИЖАЙШУЮ ДАТУ В СПИСКЕ в списке:
                data_to_return = models.AllCompetitorSiteModelDict.objects.exclude(chart_two_json={}).filter(chart_two_json__isnull=False).values_list('chart_two_json', flat=True)#.filter(chart_two_json__contains=[date_to_look_parsed_data]) # СПИСОК С ДАННЫМИ НА ДАТУ ИЗ СЛОВАРЯ  
                data_to_return = list(data_to_return)#[0]
                list_dates_keys = []    # список со всеми доступными датами
                ...
                #print('list_dates_keys', list_dates_keys)
                closest_date = min(list_dates_keys, key=lambda x: (x>date_to_look_parsed_data_indate_type, abs(x-date_to_look_parsed_data_indate_type)))
                #print('CLOSEST', closest_date, type(closest_date))
                date_to_look_parsed_data = closest_date.strftime('%d.%m.%Y')    # БЛИЖАЙШАЯ НАЙДЕННАЯ ДАТА 
                data_to_return = models.AllCompetitorSiteModelDict.objects.exclude(chart_two_json={}).filter(chart_two_json__isnull=False).values_list('chart_two_json', flat=True)#.filter(chart_two_json__contains=[date_to_look_parsed_data]) # СПИСОК С ДАННЫМИ НА ДАТУ ИЗ СЛОВАРЯ  
                data_to_return = list(data_to_return)#[0]
                for n in list(data_to_return):                  #ПОИСК НА БЛИЖАЙШУЮ НАЙДЕННУЮ ДАТУ
                    ...
                    try:
                        if n[date_to_look_parsed_data]:
                            ...
                            if data_to_return:
                                ...
                                top_brands_counter_for_chart = data_to_return[0]
                                list_of_parrsed_brands_sites = data_to_return[1]
                                #print('П--2222АПА', top_brands_counter_for_chart, date_to_look_parsed_data, list_of_parrsed_brands_sites)
                                break        
                    except: 
                        pass
                if top_brands_counter_for_chart:
                    models.CLOSEST_DATA_FOUND2 = date_to_look_parsed_data
                    models.CLOSEST_DATA_FOUND2 = date_to_look_parsed_data_indate_type
                    #print('models.CLOSEST_DATA_FOUND2 = ', models.CLOSEST_DATA_FOUND2) 
            ...
            #print('obj_with_dict ============', ) 
        # END блок по созданию отпечатка всех спарсенных данных на дату
    #print('I CHO TAM2', top_brands_counter_for_chart, date_to_look_parsed_data, list_of_parrsed_brands_sites) 
    models.CLOSEST_DATA_FOUND2 = None 
    return top_brands_counter_for_chart, date_to_look_parsed_data, list_of_parrsed_brands_sites


def chart_three_belarus(brands_on_date, min_d, max_d):
    min_date = min_d
    max_date = max_d
    #date_to_look_parsed_datas = models.DATE_TO_LOOK_PARSED_DATAS
    date_to_look_parsed_datas = brands_on_date
    date_to_look_parsed_datas = ''.join([i for i in date_to_look_parsed_datas if not i.isdigit()])
    edupl_counter = 0
    point_stop = 0
    for n in range(0, len(date_to_look_parsed_datas)):
        point_stop = n
        if date_to_look_parsed_datas[n] == ']':
            edupl_counter += 1
        if edupl_counter == 10:
            break
    date_to_look_parsed_datas = date_to_look_parsed_datas[:point_stop]
    date_to_look_parsed_datas = date_to_look_parsed_datas.replace(',', '').replace('[', '').replace('\'', '').replace(' ', '').split(']')
    #for ttyyt in date_to_look_parsed_datas:
    #    print('-----', ttyyt )     
    ...
    get_dict_wit_whole_data_on_date = models.AllCompetitorSiteModelDict.objects.exclude(chart_one_json={}).filter(chart_one_json__isnull=False).values_list('chart_one_json', flat=True)
    get_detailed_on_data_dics = models.AllCompetitorSiteModelDict.objects.exclude(chart_two_json={}).filter(chart_two_json__isnull=False).values_list('chart_two_json', flat=True)
    get_dict_wit_whole_data_on_date  = list(get_dict_wit_whole_data_on_date)
    get_detailed_on_data_dics = list(get_detailed_on_data_dics)
    for ppeeerrr in daterange_is:
        #date_to_look = ppeeerrr.date()
        period_list_of_parrsed_brands_sites = []
        brand_counter = 0
        period_str = ppeeerrr.strftime('%d.%m.%Y')
        #get_total_numb_all_on_date = None
        get_brand_numb_all_on_date = None
        for n in get_dict_wit_whole_data_on_date:
            #print('ZAS PROVERIM', n)
            n = dict(n)
            try:
                if n[period_str]:
                    all_brands_all_sites_in_date = n[period_str][0]
                    #print('ASSASIN', all_brands_all_sites_in_date)
                    break 
            except:
                pass
        for n in get_detailed_on_data_dics:
            #print('ZAS PROVERIM1', n)
            n = dict(n)
            #print('N', n)
            try:
                if n[period_str][1]:
                    get_brand_numb_all_on_date = n[period_str]
                    #print(period_str, 'ASSASIN1', get_brand_numb_all_on_date, len(get_brand_numb_all_on_date[1]), type(get_brand_numb_all_on_date[1]))              
                    for brand in date_to_look_parsed_datas:
                        try:
                            #add_len = len(brand[0])
                            add_len = len(brand)
                            #print('!!!!!!!!++!!', get_brand_numb_all_on_date[1])
                            #start_llook_at_point = get_brand_numb_all_on_date[1].rfind(brand[0]) + add_len + 2
                            start_llook_at_point = get_brand_numb_all_on_date[1].rfind(brand) + add_len + 2
                            if start_llook_at_point:
                                end_llook_at_point = get_brand_numb_all_on_date[1][start_llook_at_point:].find(']')
                                str_to_look_cut_start = get_brand_numb_all_on_date[1][start_llook_at_point:]
                                slice_brand = str_to_look_cut_start[:end_llook_at_point].replace(' ', '')
                                vlll_ll = list(slice_brand.split(',')) 
                                itogo_brand_on_date = 0 
                                for n in vlll_ll:
                                    ads = int(float(n))
                                    itogo_brand_on_date += ads  
                                brand_counter += 1
                                number_of_shown_brands_in_chart = brand_counter
                                if all_brands_all_sites_in_date and itogo_brand_on_date: 
                                    gath_data = float("{:.1f}".format(itogo_brand_on_date / all_brands_all_sites_in_date * 100 ))   # доля бренда в общем количестве на дату 
                                else:
                                    gath_data = 0
                                period_list_of_parrsed_brands_sites.append(gath_data)
                                if brand_counter == 10:                 # ограничитель топ до 10
                                    break
                        except:
                            pass
                    break # ПОД ВОПРОСОМ
        ...
        #for k, v in period_list_of_parrsed_brands_sites_dict.items():
        #    print('CHO POCHEM', k,'----',  v)  
    brands_list_for_chart_header = []
    for brand in date_to_look_parsed_datas:
        brands_list_for_chart_header.append(brand)
    dates_brands_list_for_chart_header = ['Date']                           #  списком даты
    for brand in date_to_look_parsed_datas:
        dates_brands_list_for_chart_header.append(brand)
    ...
    if second_quantity_counter == 10 or second_quantity_counter > 10:               # ели брендов более 10 - то берем то 10
        another_top_brands_counter_for_chart = 10
...
def chart_four_belarus():
    def execute_look():
        ...
        #print('==4!!!!!!!!!!!', top_tyresizes_counter_for_chart)  
        date_to_look_parsed_data = date_to_look_parsed_data.strftime('%d.%m.%Y')
        #print('DATA DATA DATA DATA', date_to_look_parsed_data)
    ...
    date_to_look_parsed_data = datetime.datetime.now().date()
    date_to_look_parsed_data.strftime('%d.%m.%Y')
    top_tyresizes_counter_for_chart = []
    date_to_look_parsed_data = []
    list_of_parrsed_tyresize_sites = []
    what_to_add_to_json = {}
    date_to_look_parsed_data = datetime.datetime.now().date()
    ...
    try:
            #блок для формирования словаря во время парсинга ------- ПАРСИНГ
        if not models.AllCompetitorSiteModelDict.objects.exclude(chart_four_json={}).filter(chart_four_json__isnull=False):    # если на автомате роходит парсинг и на дату нет - сформировать  
            ##убрать пустые объекты:
            #print('!!!!!ALARM++++!!!!!', models.AllCompetitorSiteModelDict.objects.values_list('chart_four_json', flat=True))
            #for chart_four_json_ob in models.AllCompetitorSiteModelDict.objects.filter(chart_four_json={}):
            #    chart_four_json_ob.delete()
            #    print("ALARM ALARM ALARM ALARM ALARM ALARM ALARM ALARM ALARM ALARM ALARM ALARM ")
            ## END убрать пустые объекты 
            top_tyresizes_counter_for_chart, date_to_look_parsed_data, list_of_parrsed_tyresize_sites = execute_look()
            what_to_add_to_json = {}
            what_to_add_to_json[date_to_look_parsed_data] = top_tyresizes_counter_for_chart, list_of_parrsed_tyresize_sites
            if not what_to_add_to_json:
                pass
            else:
                chart_four_json_obj, iss_created = models.AllCompetitorSiteModelDict.objects.get_or_create(chart_four_json=what_to_add_to_json) 
                chart_four_json_obj.chart_four_json = what_to_add_to_json
                chart_four_json_obj.save()
                #print('CHOTOSDELAL--=-=-=-')
    except: 
        pass
             # ЕСЛИ ДАННЫЕ ЕСТЬ   ----- НЕ ПАРСИНГ  # ВЕРНУТЬ EXCEPT
    ...
    what_to_add_to_json = {}
    try:        # ПОЛУЧИТЬ ДАННЫЕ НА ВЫБРАННУЮ ДАТУ           
        data_to_return = models.AllCompetitorSiteModelDict.objects.exclude(chart_four_json={}).filter(chart_four_json__isnull=False).values_list('chart_four_json', flat=True)#.filter(chart_four_json__contains=[date_to_look_parsed_data]) # СПИСОК С ДАННЫМИ НА ДАТУ ИЗ СЛОВАРЯ  
        #print('DATA--', date_to_look_parsed_data)
        #print('ЖЖЖ',  data_to_return)
        data_to_return = list(data_to_return)#[0]
        for n in list(data_to_return):
            ...
            try:
                if n[date_to_look_parsed_data] and n[date_to_look_parsed_data][1]:
                    ...
                    if data_to_return:
                        date_to_look_parsed_data = date_to_look_parsed_data
                        top_tyresizes_counter_for_chart = data_to_return[0]
                        list_of_parrsed_tyresize_sites = data_to_return[1]
                        #print('П--АПА', top_tyresizes_counter_for_chart, date_to_look_parsed_data, list_of_parrsed_tyresize_sites)
                        break
            except: 
                pass
    except: 
        pass
    if not top_tyresizes_counter_for_chart or not list_of_parrsed_tyresize_sites: # если на текущую дату нет данных - 1) ПОПЫТАТЬСЯ СФОРМИРОВАТЬ СЛОВАРЬ: 
        top_tyresizes_counter_for_chart, date_to_look_parsed_data, list_of_parrsed_tyresize_sites = execute_look() 
        what_to_add_to_json = {}
        what_to_add_to_json[date_to_look_parsed_data] = top_tyresizes_counter_for_chart, list_of_parrsed_tyresize_sites
        if not what_to_add_to_json or not top_tyresizes_counter_for_chart or not list_of_parrsed_tyresize_sites:
            try: # если на конкретную дату инфы нет - взять ближайшую следующую БЛИЖАЙШУЮ ДАТУ В СПИСКЕ в списке:
                data_to_return = models.AllCompetitorSiteModelDict.objects.exclude(chart_four_json={}).filter(chart_four_json__isnull=False).values_list('chart_four_json', flat=True)#.filter(chart_four_json__contains=[date_to_look_parsed_data]) # СПИСОК С ДАННЫМИ НА ДАТУ ИЗ СЛОВАРЯ  
                data_to_return = list(data_to_return)#[0]
                list_dates_keys = []    # список со всеми доступными датами
                    ...
                    try:
                        if n[date_to_look_parsed_data]:
                            ...
                            if data_to_return:
                                ...
                                top_tyresizes_counter_for_chart = data_to_return[0]
                                list_of_parrsed_tyresize_sites = data_to_return[1]
                                #print('П--002222------АПА это 4 функция', top_tyresizes_counter_for_chart, date_to_look_parsed_data, list_of_parrsed_tyresize_sites)
                                break        
                    except: 
                        pass
                if top_tyresizes_counter_for_chart:
                    #models.CLOSEST_DATA_FOUND = date_to_look_parsed_data
                    models.CLOSEST_DATA_FOUND = date_to_look_parsed_data_indate_type
                    #print('models.CLOSEST_DATA_FOUND = ', models.CLOSEST_DATA_FOUND) 
                    #print('KOT KIT', top_tyresizes_counter_for_chart, date_to_look_parsed_data, list_of_parrsed_tyresize_sites)
                    return top_tyresizes_counter_for_chart, date_to_look_parsed_data, list_of_parrsed_tyresize_sites
            except:
                pass
        else:
            ...
            #print('obj_with_dict ============', ) 
        # END блок по созданию отпечатка всех спарсенных данных на дату
    #print('I CHO TAM', top_tyresizes_counter_for_chart, date_to_look_parsed_data, list_of_parrsed_tyresize_sites) 
    models.CLOSEST_DATA_FOUND = None 
    return top_tyresizes_counter_for_chart, date_to_look_parsed_data, list_of_parrsed_tyresize_sites


def chart_five_belarus(brands_on_date, min_d, max_d):
    min_date = min_d
    max_date = max_d
    date_to_look_parsed_datas = brands_on_date
    #date_to_look_parsed_datas = ''.join([i for i in date_to_look_parsed_datas if not i.isdigit()])
    ...
    daterange_is = pd.date_range(min_date, max_date)
    number_of_shown_tyresizes_in_chart = 0             # количество показываемых в таблице типоразмеров (например, топ-5)
    get_dict_wit_whole_data_on_date = models.AllCompetitorSiteModelDict.objects.exclude(chart_one_json={}).filter(chart_one_json__isnull=False).values_list('chart_one_json', flat=True)
    get_detailed_on_data_dics = models.AllCompetitorSiteModelDict.objects.exclude(chart_four_json={}).filter(chart_four_json__isnull=False).values_list('chart_four_json', flat=True)
    get_dict_wit_whole_data_on_date = list(get_dict_wit_whole_data_on_date)
    get_detailed_on_data_dics = list(get_detailed_on_data_dics)
    for ppeeerrr in daterange_is:
        #date_to_look = ppeeerrr.date()
        period_list_of_parrsed_tyresizes_sites = []
        tyresize_counter = 0
        period_str = ppeeerrr.strftime('%d.%m.%Y')
        #get_total_numb_all_on_date = None
        get_tyresize_numb_all_on_date = None
        for n in get_dict_wit_whole_data_on_date:
            #print('ZAS PROVERIM', n)
            n = dict(n)
            try:
                if n[period_str]:
                    all_tyresizes_all_sites_in_date = n[period_str][0]
                    #print('ASSASIN', all_tyresizes_all_sites_in_date)
                    break 
            except:
                pass
        for n in get_detailed_on_data_dics:
            ...
            try:
                if n[period_str][1]:
                    get_tyresize_numb_all_on_date = n[period_str]
                    #print(period_str, 'ASSASIN1', get_tyresize_numb_all_on_date, len(get_tyresize_numb_all_on_datee[1]), type(get_tyresize_numb_all_on_date[1]))              
                    for brand in date_to_look_parsed_datas:
                        try:
                            #add_len = len(brand[0])
                            add_len = len(brand)
                            #print('!!!!!!!!++!!', get_tyresize_numb_all_on_date[1])
                            #start_llook_at_point = get_tyresize_numb_all_on_date[1].rfind(brand[0]) + add_len + 2
                            start_llook_at_point = get_tyresize_numb_all_on_date[1].rfind(brand) + add_len + 2
                            if start_llook_at_point:
                                end_llook_at_point = get_tyresize_numb_all_on_date[1][start_llook_at_point:].find(']')
                                str_to_look_cut_start = get_tyresize_numb_all_on_date[1][start_llook_at_point:]
                                slice_tyresize = str_to_look_cut_start[:end_llook_at_point].replace(' ', '')
                                vlll_ll = list(slice_tyresize.split(',')) 
                                itogo_tyresize_on_date = 0 
                                for n in vlll_ll:
                                    ads = int(float(n))
                                    itogo_tyresize_on_date += ads  
                                tyresize_counter += 1
                                number_of_shown_tyresizes_in_chart = tyresize_counter
                                if all_tyresizes_all_sites_in_date and itogo_tyresize_on_date: 
                                    gath_data = float("{:.1f}".format(itogo_tyresize_on_date / all_tyresizes_all_sites_in_date * 100 ))   # доля бренда в общем количестве на дату 
                                else:
                                    gath_data = 0
                                period_list_of_parrsed_tyresizes_sites.append(gath_data)
                                if tyresize_counter == 10:                 # ограничитель топ до 10
                                    break
                        except:
                            pass
                    break # ПОД ВОПРОСОМ
        ...
        #for k, v in period_list_of_parrsed_tyresizes_sites_dict.items():
        #    print('CHO POCHEM', k,'----',  v)  
    tyresizes_list_for_chart_header = []
    for tyresize in date_to_look_parsed_datas:
        tyresizes_list_for_chart_header.append(tyresize)
    dates_tyresizes_list_for_chart_header = ['Date']                           #  списком даты
    for tyresize in date_to_look_parsed_datas:
        dates_tyresizes_list_for_chart_header.append(tyresize)
    second_quantity_counter = number_of_shown_tyresizes_in_chart 
    another_top_tyresize_counter_for_chart = 0
    if second_quantity_counter == 10 or second_quantity_counter > 10:               # ели брендов более 10 - то берем то 10
        another_top_tyresize_counter_for_chart = 10
    elif second_quantity_counter < 10 and second_quantity_counter > 0:
...
def belarus_sites_parsing():
    ...
    is_now_today = datetime.datetime.today().date()
    data_is_parsed_on_date = models.CompetitorSiteModel.objects.filter(date_period=is_now_today).exists()
    if data_is_parsed_on_date is False:                                                                          # если на сегодня нет спаршенныз данных - то парсить
        ...
        #for ttyy in models.ComparativeAnalysisTyresModel.objects.all():
        #    print(ttyy.tyre.tyre_size,  ttyy.tyre.tyre_type.all())
            ...
            for k, v in goods_dict.items():
                ...
                if tyre.tyre_size.tyre_size == v[1]:                                                                                            #  ПРОСМОТР ВСЕХ СПАРСЕННЫХ 
                  ...
                  ##     print('HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH', v[6])
                    ...
                    # ДОП ФИЛЬТР НА ПОЛНОЕ СОВПАДЕНЕ С ШИНАМИ БЕЛШИНЫ ПО ПАРАМЕТРАМ- ЧТОБЫ НЕ ВНОСИТЬ В БАЗУ ЛИШНЕЕ
                    ...
                    if models.ComparativeAnalysisTyresModel.objects.filter(tyre__tyre_size__tyre_size=v[1],  tyre__tyre_group__tyre_group=v[6]):                
                    # ДОП END ФИЛЬТР НА ПОЛНОЕ СОВПАДЕНЕ С ШИНАМИ БЕЛШИНЫ ПО ПАРАМЕТРАМ- ЧТОБЫ НЕ ВНОСИТЬ В БАЗУ ЛИШНЕЕ  
                        comptyreswithsameparam = models.ComparativeAnalysisTyresModel.objects.filter(tyre__tyre_size__tyre_size=v[1], tyre__tyre_group__tyre_group=v[6]).all()  
                        matched_tyre = False 
                        for cccomp in comptyreswithsameparam: 
                            #print(cccomp.tyre.tyre_group.all(), 'cccomp', cccomp.tyre.tyre_type.all())
                            pass
                            #while matched_tyre is False:
                            #    for written_param in cccomp.tyre.tyre_type.all():
                            #        #print(v[3], '==IIIIIII-===========', written_param)
                            #        if v[3] == written_param:
                            #            print(v[3], '++++666+++', written_param)
                ...
                for tyre in tyres_in_bd:
                    try:
                        ...
                        #    print(k, 'GGG baGGor', v, 'GGG', len(v))
                            if tyre.tyre_size.tyre_size == k[0]:
                            #    print('TTTT', k, 's111', v)           TTTT ('155/65R13', 431) s111 ('WestLake', 'SW618', '73T', 'зимние', 'легковые', '126.07')   
                                #print('!!!!04040!!!', v[2], '---', v[3])                                                                               #  ПРОСМОТР ВСЕХ СПАРСЕННЫХ 
...
#        finally:
        ...
        pass   # если есть спаршенные данные на сегодня- то прпускаем операцию                                                                                                                                                                                      
...
#            goods_dict_express_shina_dopolnitelno = {} 
            ...
            #pages_num_list = [[0, 1],[1, 1]]                            # CТРАНИЦЫ
            pages_num_list = [[0, 1]]  
            #!!!!!!!!!!!
   ...
   #        3) парсинг спец шин
            ...
            #for slug in range(1, urls_get[-1]):                             # мое добавление специально для express-shina  # c 1 по 2 станицы
            for slug in range(1, 2):
                print('page - парсинг спец шин', slug )
    ...
    #        print('!!!!======++', 'urls_get', urls_get)
        ...
        #            print('page - парсинг легковых зимних шин', slug )
            ...
            #2. получаем данные со всех страниц:                         
            #for slug in range(1, urls_get[-1]):                             # мое добавление специально для express-shina  # c 1 по 2 станицы
            for slug in range(1, 4):
            #    print('page - парсинг легковых летних шин', slug )
                newUrl = url + f'nav/page-{slug}/'       #https://www.kolesa-darom.ru/catalog/avto/shiny/zima/nav/page-2/
                webdriverr.get(newUrl)
                time.sleep(2)
                webdriverr.execute_script("window.scrollTo(0, document.body.scrollHeight);")
       ...
       #             print(k, 'GGG', v, 'GGG', len(v))
        ...
        pass            # если есть данные спаршенные на сегодня- пропускаем       
...
class ComparativeAnalysisTableModelDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs): 
        start_time_CHECK = time.time()    # УДАЛИТЬ по итогу - просто для проверкискорости выполнения проги
...
#    list_of_tyre_comparative_objects = table_lookup_only_with_competitors.filter(sale_data__year=year_to_look, sale_data__month=month_to_look)     # проверенный но МЕДЛЕННЫЙ вариант - берем все объекты на посл дату и работаем по ним       
    ...
    ##                        print('!!!!GGGG! = ', list_of_tyre_comparative_objects) 
        ...
        print('list_of_tyre_comparative_objects-111 ', list_of_tyre_comparative_objects)
...
####################
    ...
    #        print('GGGOOODDD 3', context['list_of_competitor_values_new'])
        ...
        # ФИЛЬТР даты для кругового и 4-х графиков:
        if models.COMPETITORS_DATE_FROM_USER_ON_FILTER:
            date_to_look_parsed_data_is = models.COMPETITORS_DATE_FROM_USER_ON_FILTER[0]
            date_to_look_parsed_data_is = datetime.datetime.strptime(date_to_look_parsed_data_is, '%Y-%m-%d').date()
            date_to_look_parsed_data_is = date_to_look_parsed_data_is.strftime('%d.%m.%Y')

        #### КРУГОВОЙ ГРАФИК КОЛИЧЕСТВО СПАРСЕННЫХ ДАННЫХ С САЙТА: PANDAS
        if not models.FINAL_PARSED_DATA_FROM_SITES or not models.FINAL_PARSED_DATA_FROM_SITES_DATA or models.FINAL_PARSED_DATA_FROM_SITES_DATA != date_to_look_parsed_data_is: #если еще не расчитывались данные на сегодня - подготовить данные
            # СПЕЦИАЛЬНЫЙ ОТРЕЗОК - исключение - вернуть ранее подготовленные данные данные без повторного расчета (расчитано ниже) в случве подготовки данных на ближайшую дату (на выбранную дату данных нет)
            #if models.CLOSEST_DATA_FOUND1 and models.CLOSEST_DATA_FOUND1 == datetime.datetime.strptime(date_to_look_parsed_data_is, '%d.%m.%Y').date():  
            if models.FINAL_PARSED_DATA_FROM_SITES_WHOLE and models.CLOSEST_DATA_FOUND1 == datetime.datetime.strptime(date_to_look_parsed_data_is, '%d.%m.%Y').date():     
                #print("1+_+_+_", models.CLOSEST_DATA_FOUND1, '$$$$$', datetime.datetime.strptime(date_to_look_parsed_data_is, '%d.%m.%Y').date(), '$$$$$')
                ...
                #print('ДАННЫЕ ДАНЫ НА БЛИЖАЙШУЮ ДАТУ БЕЗ ПОВТОРНОГО РАСЧЕТА')
            # END СПЕЦИАЛЬНЫЙ ОТРЕЗОК - исключение - вернуть ранее подготовленные данные данные без повторного расчета (расчитано ниже) в случве подготовки данных на ближайшую дату (на выбранную дату данных нет)              
            else:    
                models.CLOSEST_DATA_FOUND1 = None
                final_parsed_data_from_sites_whole, final_parsed_data_from_sites,  date_to_look_parsed_data = chart_one_belarus()
                #print('A1111111AAAAAAAAAAAAAAAAAAAAAAAAAAA')
            ...
            models.CLOSEST_DATA_FOUND1 = None  
            #print('B111111BBBBBBBBBBBBBBBBBBBBBBBBBBBBB')                                                                                     # если уже на сегодня расчитано - взять готовые данные
            context['final_parsed_data_from_sites_whole'] = models.FINAL_PARSED_DATA_FROM_SITES_WHOLE
            context['final_parsed_data_from_sites'] = models.FINAL_PARSED_DATA_FROM_SITES
            context['final_parsed_data_from_sites_data'] = models.FINAL_PARSED_DATA_FROM_SITES_DATA
            #print("HELL YEHA---")
        ...
        if not models.BRANDS_FROM_SITES_DATE or not models.BRANDS_FROM_SITES_DATE or models.BRANDS_FROM_SITES_DATE != date_to_look_parsed_data_is:                #если еще не расчитывались данные на сегодня - подготовить данные
            # СПЕЦИАЛЬНЫЙ ОТРЕЗОК - исключение - вернуть ранее подготовленные данные данные без повторного расчета (расчитано ниже) в случве подготовки данных на ближайшую дату (на выбранную дату данных нет)
            #if models.CLOSEST_DATA_FOUND2 and models.CLOSEST_DATA_FOUND2 == datetime.datetime.strptime(date_to_look_parsed_data, '%d.%m.%Y').date(): 
            if models.TOP_BRANDS_NUM and models.CLOSEST_DATA_FOUND2 == datetime.datetime.strptime(date_to_look_parsed_data_is, '%d.%m.%Y').date():   
                #print("2+_+_+_", models.CLOSEST_DATA_FOUND2, '$$$$$', datetime.datetime.strptime(date_to_look_parsed_data_is, '%d.%m.%Y').date(), '$$$$$')
                context['brands_from_sites'] = models.BRANDS_FROM_SITES
            ...
            # END СПЕЦИАЛЬНЫЙ ОТРЕЗОК - исключение - вернуть ранее подготовленные данные данные без повторного расчета (расчитано ниже) в случве подготовки данных на ближайшую дату (на выбранную дату данных нет)              
                ...
                #print('A222222AAAAAAAAAAAAAAAAAAAAAAAAAAA')
                ...
                models.TOP_BRANDS_NUM = context['top_brands_num']
        else: 
            models.CLOSEST_DATA_FOUND2 = None
            #print('B222222BBBBBBBBBBBBBBBBBBBBBBBBBBBBB')                                                                      # если уже на сегодня расчитано - взять готовые данные
            context['brands_from_sites'] = models.BRANDS_FROM_SITES
            context['brands_from_sites_date'] = models.BRANDS_FROM_SITES_DATE
            context['top_brands_num'] = models.TOP_BRANDS_NUM
            #print("HELL YEHA+++")
        ...
        #elif #not models.TYRESIZES_FROM_SITES_DATE or not models.TYRESITES_FROM_SITES or models.TYRESIZES_FROM_SITES_DATE != date_to_look_parsed_data_is or models.CLOSEST_DATA_FOUND == None:
        if not models.TYRESIZES_FROM_SITES_DATE or not models.TYRESITES_FROM_SITES or models.TYRESIZES_FROM_SITES_DATE != date_to_look_parsed_data_is:
            # СПЕЦИАЛЬНЫЙ ОТРЕЗОК - исключение - вернуть ранее подготовленные данные данные без повторного расчета (расчитано ниже) в случве подготовки данных на ближайшую дату (на выбранную дату данных нет)   
            if models.TOP_TYRESIZES_NUM and models.CLOSEST_DATA_FOUND == datetime.datetime.strptime(date_to_look_parsed_data_is, '%d.%m.%Y').date():
                #print("+_+_+_ZOPA", models.CLOSEST_DATA_FOUND, '$$$$$', datetime.datetime.strptime(date_to_look_parsed_data_is, '%d.%m.%Y').date(), '$$$$$')
                context['tyresizes_from_sites_date'] = models.TYRESIZES_FROM_SITES_DATE
                context['tyresizes_from_sites'] = models.TYRESITES_FROM_SITES
            ...
            # END СПЕЦИАЛЬНЫЙ ОТРЕЗОК - исключение - вернуть ранее подготовленные данные данные без повторного расчета (расчитано ниже) в случве подготовки данных на ближайшую дату (на выбранную дату данных нет)  
                ...
                models.TOP_TYRESIZES_NUM = context['top_tyresizes_num']  
        else:
            models.CLOSEST_DATA_FOUND = None
...
######        print('brand_names_check_status_list1111', brand_names_check_status_list)
    ...
    #        print('END 0 ===== list_of_competitor_values_new_dict', list_of_competitor_values_new_dict)
        ...
        #### КРУГОВОЙ ГРАФИК КОЛИЧЕСТВО СПАРСЕННЫХ ДАННЫХ С САЙТА: PANDAS
        if not models.FINAL_PARSED_DATA_FROM_SITES_WHOLE_RUSSIA or not models.FINAL_PARSED_DATA_FROM_SITES_RUSSIA: #если еще не расчитывались данные на сегодня - подготовить данные
            final_parsed_data_from_sites = []
            parsed_data_from_sites = ['Сайт', 'Количество спарсенных конкурентов']
            final_parsed_data_from_sites.append(parsed_data_from_sites)
            ...
            context['final_parsed_data_from_sites'] = final_parsed_data_from_sites
            context['final_parsed_data_from_sites_data'] = date_to_look_parsed_data
            models.FINAL_PARSED_DATA_FROM_SITES_WHOLE_RUSSIA = context['final_parsed_data_from_sites_whole']
            models.FINAL_PARSED_DATA_FROM_SITES_RUSSIA = context['final_parsed_data_from_sites']
            models.FINAL_PARSED_DATA_FROM_SITES_DATA_RUSSIA = context['final_parsed_data_from_sites_data']
        else:
            context['final_parsed_data_from_sites_whole'] = models.FINAL_PARSED_DATA_FROM_SITES_WHOLE_RUSSIA 
            context['final_parsed_data_from_sites'] = models.FINAL_PARSED_DATA_FROM_SITES_RUSSIA
            context['final_parsed_data_from_sites_data'] = models.FINAL_PARSED_DATA_FROM_SITES_DATA_RUSSIA           
        #### END  КРУГОВОЙ ГРАФИК КОЛИЧЕСТВО СПАРСЕННЫХ ДАННЫХ С САЙТА: PANDAS

        #### ГРАФИК КОЛИЧЕСТВО СПАРСЕННЫХ ДАННЫХ ПО БРЕНДУ С САЙТОВ: PANDAS
        if not models.BRANDS_FROM_SITES_DATE_RUSSIA or not models.BRANDS_FROM_SITES_RUSSIA: #если еще не расчитывались данные на сегодня - подготовить данные
            all_parsed_brands_developers_queryset = dictionaries_models.CompetitorModel.objects.order_by('competitor_name').values_list('competitor_name', flat=True).distinct()        ### Фильтр уникальных!
            date_to_look_parsed_data = datetime.datetime.now().date()
            if models.COMPETITORS_DATE_FROM_USER_ON_FILTER:
                date_to_look_parsed_data = models.COMPETITORS_DATE_FROM_USER_ON_FILTER[0]
            ...
            context['brands_from_sites'] = list_of_parrsed_brands_sites
            models.BRANDS_FROM_SITES_DATE_RUSSIA = context['brands_from_sites_date']
            models.BRANDS_FROM_SITES_RUSSIA = context['brands_from_sites']
            models.TOP_BRANDS_NUM_RUSSIA = context['top_brands_num']
        else:               # если уже на сегодня расчитано - взять готовые данные
            context['brands_from_sites_date'] = models.BRANDS_FROM_SITES_DATE_RUSSIA 
            context['brands_from_sites'] = models.BRANDS_FROM_SITES_RUSSIA
            context['top_brands_num']= models.TOP_BRANDS_NUM_RUSSIA 


        # ГРАФИК ДИНАМИКА ТОП БРЕНДОВ ИСХОДЯ ИЗ ПОСЛЕДНЕЙ ДАТЫ:
        if not models.PERIOD_LIST_OF_PARSED_BRANDS_SITES_DICT_RUSSIA or not models.DATES_BRANDS_LIST_FOR_CHART_HEADER_RUSSIA: #если еще не расчитывались данные на сегодня - подготовить данные
            period_list_of_parrsed_brands_sites_dict = {}
            daterange_is = pd.date_range(min_date, max_date)
            number_of_shown_brands_in_chart = 0             # количество показываемых в таблице брендов (например, топ-5)
            for ppeeerrr in daterange_is:
            ...
            if second_quantity_counter == 10 or second_quantity_counter > 10:               # ели брендов более 10 - то берем то 10
                ...
                another_top_brands_counter_for_chart = 'лист без данных'
            context['second_top_brands_num'] = another_top_brands_counter_for_chart  
            models.PERIOD_LIST_OF_PARSED_BRANDS_SITES_DICT_RUSSIA = context['period_list_of_parrsed_brands_sites_dict']
            models.DATES_BRANDS_LIST_FOR_CHART_HEADER_RUSSIA = context['dates_brands_list_for_chart_header']
            models.TOP_BRANDS_NUM_RUSSIA = context['second_top_brands_num']
        else:                       # если уже на сегодня расчитано - взять готовые данные
            context['period_list_of_parrsed_brands_sites_dict'] = models.PERIOD_LIST_OF_PARSED_BRANDS_SITES_DICT_RUSSIA
            context['dates_brands_list_for_chart_header'] = models.DATES_BRANDS_LIST_FOR_CHART_HEADER_RUSSIA
            context['second_top_brands_num'] = models.TOP_BRANDS_NUM_RUSSIA

    #    print(context['dates_brands_list_for_chart_header'])
    #    print(context['period_list_of_parrsed_brands_sites_dict'])

        #for n, v in period_list_of_parrsed_brands_sites_dict.items():
        #    print(n, v)
    ...
    #    all_parsed_tyresizes_developers_queryset = dictionaries_models.TyreSizeModel.objects.order_by('tyre_size').values_list('tyre_size', flat=True).distinct()        ### Фильтр уникальных!
        if not models.TYRESIZES_FROM_SITES_DATE_RUSSIA or not models.TYRESITES_FROM_SITES_RUSSIA:  #если еще не расчитывались данные на сегодня - подготовить данные
            all_parsed_tyresizes_developers_queryset = models.CompetitorSiteModel.objects.order_by('tyresize_competitor').values_list('tyresize_competitor', flat=True).distinct()        ### Фильтр уникальных!
            date_to_look_parsed_data = datetime.datetime.now().date()
            if models.COMPETITORS_DATE_FROM_USER_ON_FILTER:
                date_to_look_parsed_data = models.COMPETITORS_DATE_FROM_USER_ON_FILTER[0]
            ...
            context['tyresizes_from_sites'] = list_of_parrsed_tyresize_sites
            models.TYRESIZES_FROM_SITES_DATE_RUSSIA = context['tyresizes_from_sites_date']
            models.TYRESITES_FROM_SITES_RUSSIA = context['tyresizes_from_sites']
            models.TOP_TYRESIZES_NUM_RUSSIA = context['top_tyresizes_num']
        else:    # если уже на сегодня расчитано - взять готовые данные
            context['tyresizes_from_sites_date'] = models.TYRESIZES_FROM_SITES_DATE_RUSSIA 
            context['tyresizes_from_sites'] = models.TYRESITES_FROM_SITES_RUSSIA
            context['top_tyresizes_num']= models.TOP_TYRESIZES_NUM_RUSSIA


        # ГРАФИК ДИНАМИКА ТОП БРЕНДОВ ИСХОДЯ ИЗ ПОСЛЕДНЕЙ ДАТЫ:
        if not models.PERIOD_LIST_OF_PARSED_TYRETYPES_DICT_RUSSIA or not models.DATES_TYRESIZES_LIST_FOR_CHART_HEADER_RUSSIA: #если еще не расчитывались данные на сегодня - подготовить данные
            period_list_of_parrsed_tyresizes_sites_dict = {}
            daterange_is = pd.date_range(min_date, max_date)
            ...
            for ppeeerrr in daterange_is:
                ...
                period_list_of_parrsed_tyresizes_sites = []
                tyresize_counter = 0
                all_tyresizes_all_sites_in_date = models.CompetitorSiteModel.objects.filter(date_period=date_to_look, site__in=['express-shina.ru', 'kolesa-darom.ru', 'kolesatyt.ru']).count()      # всего все бренды на дату
                #print('all_tyresizes_all_sites_in_date', all_tyresizes_all_sites_in_date)
                for tyresize in list_of_parrsed_tyresize_sites1:
                #    print('tyresize', tyresize[0])
                    num_of_parsed_tyresize_express_shina = models.CompetitorSiteModel.objects.filter(tyresize_competitor=tyresize[0], date_period=date_to_look, site='express-shina.ru').count()
                    num_of_parsed_tyresize_kolesa_darom = models.CompetitorSiteModel.objects.filter(tyresize_competitor=tyresize[0], date_period=date_to_look, site='kolesa-darom.ru').count()
                    num_of_parsed_tyresize_kolesatyt = models.CompetitorSiteModel.objects.filter(tyresize_competitor=tyresize[0], date_period=date_to_look, site='kolesatyt.ru').count() 
                ...
                #    gath_data =  total_quantity
                    ...
                    #    print('total_quantity', total_quantity)
                        gath_data = float("{:.1f}".format(total_quantity)), float("{:.1f}".format(total_quantity / all_tyresizes_all_sites_in_date * 100))
                    else:
                        gath_data = 0
            ...
            #for v, nn in period_list_of_parrsed_tyresizes_sites_dict.items():          # значения брендов суммарные списком на дату
            #    nn.insert(0, v)
            dates_tyresizes_list_for_chart_header = ['Date']                           #  списком даты
            for trsize in list_of_parrsed_tyresize_sites1:
            ...
            if second_quantity_tyresizes_counter == 10 or second_quantity_tyresizes_counter > 10:               # ели брендов более 10 - то берем то 10
                ...
                another_top_tyresize_counter_for_chart = 'лист без данных'        
            context['second_top_tyresize_num'] = another_top_tyresize_counter_for_chart 
            models.PERIOD_LIST_OF_PARSED_TYRETYPES_DICT_RUSSIA = context['period_list_of_parrsed_tyresizes_sites_dict']
            models.DATES_TYRESIZES_LIST_FOR_CHART_HEADER_RUSSIA = context['dates_tyresizes_list_for_chart_header']
            models.SECOND_TOP_TYRESIZE_NUM_RUSSIA = context['second_top_tyresize_num']
        else:                                                               # если уже на сегодня расчитано - взять готовые данные
            context['period_list_of_parrsed_tyresizes_sites_dict'] = models.PERIOD_LIST_OF_PARSED_TYRETYPES_DICT_RUSSIA
            context['dates_tyresizes_list_for_chart_header'] = models.DATES_TYRESIZES_LIST_FOR_CHART_HEADER_RUSSIA
            context['second_top_tyresize_num'] = models.SECOND_TOP_TYRESIZE_NUM_RUSSIA
